...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.7\linewidth]{Figures/Ontology/SubjectExecution/ASM-Behavior}
	\caption[ASM Code for Behavior]{ASM Code for Behavior}
	\label{fig:asm-behavior}
\end{figure}

Because of historical reasons in the ASM the names of the objects in the ontology are not used. Table \ref{tab:places} shows the mapping of the variable names used in the ASM (in ASM also called places) to the ontology object names. The names printed in \textit{italic }in the column "ASM-Interpreter" corespond to the \textbf{bold} printed names in the column "OWL Model".
The OWL model element \textbf{State} coresponds to the place \textit{SID\_state} or \textit{D} is mapped to \textbf{SubjectBehaviour}.
...
\begin{landscape}
	...
	\textbf{\textit{DefaultFunctionReceive1\_-EnvironmentChoice \newline
			DefaultFunctionReceive2\_-AutoReceiveEarliest \newline
			DefaultFunctionSend }}
	& \textit{ComAct} 
	...
	\\
	\hline
	\caption{Mapping of ASM places to objects of the ontology}
	\label{tab:places}
...
be defined in terms of the ASM submachines START and PERFORM together with the
...
>>>>>>> 9542de488102928f002a500d8df07552608d9983
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.7\linewidth]{Figures/Ontology/SubjectExecution/ASM-perform}
	\caption{}
