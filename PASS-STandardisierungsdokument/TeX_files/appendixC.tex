...
\begin{listing}[H]
\begin{minted}[fontsize=\small]{lexer.py:CoreASMLexer -x}
rule ReceiveMessage(MI, currentStateNumber, t) =
  let ch = channelFor(self),
     pID = processIDFor(self),
       s = currentStateNumber in
  let sID      = messageSubjectId         (pID, t),
      ...
      countMin = messageSubjectCountMin   (pID, t),
      countMax = messageSubjectCountMax   (pID, t) in
  let cID = loadCorrelationID(MI, cIDVName) in {
    seq
      // stores the messages in receivedMessages
      InputPool_Pop(MI, s, sID, mT, cID, countMin, countMax)
    next
...
rule CheckIP(MI, currentStateNumber, t) = {
  let pID   = processIDFor(self) in
  let sID       = messageSubjectId         (pID, t),
      sChsVName = messageSubjectVar        (pID, t),
      mT        = messageType              (pID, t),
      cIDVName  = messageWithCorrelationVar(pID, t) in
  ...
  seqblock
      ...
      if (cIDVName = "?") then {
        ...
        numres1 := loadCorrelationID(MI, messageWithCorrelationVar(pID, t))
      }
      ...
      let usedSpace = inputPoolUsedSpace(channelFor(self), stringres1, setres1, stringres2, numres1, true, true) in {
        if (messageSubjectCountMin(pID, t) = 0) then {
        ...
        else if (usedSpace >= messageSubjectCountMin(pID, t)) then {
          debuginfo CheckIP self + ": enough messages :) " + usedSpace + " >= " + messageSubjectCountMin(pID, t)
          EnableTransition(MI, t)
        }
        else {
          debuginfo CheckIP self + ": Not enough messages: " + usedSpace + " < " + messageSubjectCountMin(pID, t)
          DisableTransition(MI, currentStateNumber, t)
        }
      }
  endseqblock
