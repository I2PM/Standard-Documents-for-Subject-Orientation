...
rule Behaviour(MI, currentStateNumber) =
 let ch = channelFor(self) in
  if (initializedState(ch, MI, currentStateNumber) != true) then
    StartState(MI, currentStateNumber)
  else if (abortState(MI, currentStateNumber) = true) then
    AbortState(MI, currentStateNumber)
  else if (completed(ch, MI, currentStateNumber) != true) then
    Perform(MI, currentStateNumber)
  else if (initializedSelectedTransition(ch, MI, currentStateNumber) != true) then
    StartSelectedTransition(MI, currentStateNumber)
  else
   let transitionNumber = selectedTransition(ch, MI, currentStateNumber) in
    if (transitionCompleted(ch, MI, transitionNumber) != true) then
      PerformTransition(MI, currentStateNumber, transitionNumber)
    else {
      Proceed(MI, currentStateNumber, targetStateNumber(processIDFor(self), transitionNumber))
