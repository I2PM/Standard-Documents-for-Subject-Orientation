...
\begin{list}{-}
	...
	\item Business Activity Monitoring for S-BPM: \cite{article:SBPMCosting}
...
In section II we review related research to developing fog applications according to stakeholder needs in various domains. Section III introduces SFC based on a System-of-Systems perspective, and provides an exemplary case from developing home healthcare support systems. Section IV concludes the paper summarizing SFC and indicating further research. 
...
\end{list}
Our future research will focus on including network configuration information into the subject-oriented behavior abstractions, in order to optimize FC applications. The objective is to qualify end users for fine tuning their setting according to their needs and available Fog actors. Once they are able to edit and validate the subject behavior models, they also can deal with organizational and technical implementation details, allowing them to adapt an entire application as System-of-System dynamically. Adaptation to new policies can be implemented in this way (cf. \cite{article:SecurityMgmt}), leading to more situation-sensitive Fog applications (cf. \cite{article:FogSimToolKit}). 


\section{Activity Based Costing}

1   Introduction
Since the 1980ies the importance of process orientation in business is steadily growing. In a study on the status quo of business process management (BPM) conducted in Germany and Austria in 2011 the authors conclude though ‘The enterprises highly value BPM, but its implementation is, critically, still in its childhood’. Current focus is on systematically documenting business processes, which is an important first step, but still far from reaching the objective. Only a few companies consequently perform process performance measurement with well-defined key figures, being the sole base for continuous process improvement.’[14, p. 10].
Facts like this motivated our contribution elaborating a concept to integrate process controlling functionality into the methodology of Subject-oriented BPM (S-BPM) and into software supporting the approach. The focus is on providing cost information about processes [19].
After this introduction in chapter 2 we briefly introduce process controlling and the concept of Activity-Based Costing (ABC) before we present S-BPM as a supplier of raw data to generate management information from. The example in chapter 3 illustrates how the so far developed ideas could be implemented. The contribution ends with a short summary and some thoughts of future work.
2   Basic Concepts
2.1   Process Controlling
Process controlling has both a strategic and an operational dimension [cf. e.g. 15, p. 229 ff.]. We concentrate on methods and techniques for planning, designing and coordinating the supply of information necessary to allow continuous operational process controlling with key figures as indicated in the closed-loop approach to performance management (see lower part of figure 1). As operational process controlling aims for post-execution analysis of business process instances it can be complemented by Business Activity Monitoring (BAM) which, based on event processing concepts, observes instances during execution and sets alerts or triggers actions in real-time or near real-time according to the particular situations identified [cf. 16, 10].


Fig. 1 Closed-loop Approach to Performance Management [5]

The major question is how to measure process performance. A typical parameter for the evaluation of process effectivity is customer satisfaction while process time, quality and cost and adherence to schedules are suitable to assess efficiency [cf. e.g. 15 p. 229 ff.]. As these parameters have high significance for the competitive position they are crucial for process controlling. While the assessment of customer satisfaction and maturity levels of processes usually are matters of periodic monitoring activities there might be other, more technical parameters needed to be watched permanently, like the response time of application systems. Those aspects are especially relevant if processes are extensively supported by IT.
Figure 2 gives a conceptual overview of a key figure-based operational process controlling, split into continuous and periodic or occasional controlling activities, like it could be set up successively for the S-BPM approach.
The integrated collection and analysis of common managerial data allows for a cohesive evaluation and control in terms of process controlling [cf. 15 p. 248 ff., 1 p. 385 ff., 7 p. 158 ff.]. It feeds back results to take decisions and actions, fostering a steadily growth of experience regarding the interdependencies between cost, quality and time (organizational learning).


Fig. 2.  Operational Process Controlling

In this paper we focus on cost figures. Calculating them is more difficult than assessing those for time and quality. S-BPM allows determining cost figures for processes and process steps as well as for the occupation of cost centers and organizational units with little additional effort though. The reason is that S-BPM specifies subjects as actors in a process, their interaction and their assignment to elements of the organizational structure (organizational units, positions, roles) (see section 2.3). 
The basic methodology to integrate such cost information into process controlling is Activity-Based Costing (ABC).
2.2   Methodology of Activity-Based Costing
The concept of Activity-Based Costing origins in the work of Miller and Vollmann [12] and Cooper and Kaplan [3] and was established in the German-speaking community by Horvath und Mayer [9].
...
The analysis results in a transparent, hierarchical process structure showing the assignment of activities to process steps, the assignment of process steps to cost centers and the aggregation of process steps to main processes (cf. figure 3)
...
This means the workflow system generates a valuable data basis for a meaningful Activity-Based Costing. This data needs to be categorized though and the key figures need to be defined precisely and unique in order to derive useful management information (see example in section 3).
3   Example for Estimating Process Costs in S-BPM
...
For estimating the costs of the process we need an hourly or minute-related rate of wage for the people being assigned to the subjects. It is also possible to provide those values aggregated on group or role level. In Figure 5 we visualized how the subjects in our example are mapped to persons, while table 2 gives an overview of the rates for the employees involved as subject representatives.



Fig. 5.  Embedding the Subjects into the Organization
Table 2.  Hourly Wage Rates
Employee	Wage rate
Miller, Laurel, Schulz	200 €/hour
Ployer, Schmidt, Doe, Keweling	100 €/hour
Weber, Wagner, Kramer, Meier, Switch, Lampe, Smith, Stuart, Buchner, Richards, Thorwald, Regal, Mustermann, Jones	50 €/hour

Having these time and wage values available it is a simple multiplication to determine the personnel-related process costs for every single instance (cf. table 3). Considering a sufficient number of instances over a representative period of time allows computing a valid average cost value.
Table 3.  Personnel-related Process Costs
Subject	Employee	Costs
Purchaser	Kramer	35 Min. x 50 €/60 min.  =  29,17 €
Approver	Ployer	10 Min. x 100 €/60 min. =  16,67 €
Warehouse	Lampe	30 Min. x 50 €/60 min.  =  25,00 €
Invoice verification	Regal	5 Min. x 50 €/60 min.    =    4,17 €
Accounting/Book keeping	Regal	5 Min. x 50 €/60 min.    =    4,17 €
Total		79,18 €

Assigning employees as subject representatives embeds the subjects into the organizational structure, because people belong to organizational units. As cost centers usually also are assigned to organizational units it is now possible to determine the costs of a process incurring in a certain cost center. From the cost center perspective it is also possible to see how its total costs are distributed over the processes and process steps it is involved in. Table 4 shows examples for cost figures which can be computed.
Table 4.  Cost Figures
Key figure (€)	Computation (e.g. for 10 work days)
...
∑ completed instances
Data sources
(general)	Tables in the database of Metasonic Suite:
RT_PROCDESC, RT_PROCINST, REC_PARADESC, REC_RECTRANS, UM_USER
Data sources
(specific)	Processing time:
SELECT TIMESTAMP1 – 
(SELECT STARTTIME
FROM RT_PROCINST
WHERE RT_PROCDESC = process
AND ID = instance
FROM REC_RECTRANS
WHERE RT_STDESC = state
AND RT_PROCINST = instance
Hourly wage rate: UM_USER (manually enriched by hourly wage rates)
Completed instances: see separate fact sheet
Frequency	weekly
Presentation
...
17.	Schmidt, W., Fleischmann, A.: Business Process Monitoring with S-BPM, in: Fischer, H., Schneeberger, J. (Eds.), Proceedings of the 5th International Conference S-BPM ONE 2013, Communications in Computer and Information Sciences (CCIS), No. 360, Springer, Berlin Heidelberg 2013.
18.	Schmidt, W., Fleischmann, A. und Gilbert, O.: Subjektorientiertes Geschäftsprozess-management, HMD – Praxis der Wirtschaftsinformatik, Heft 266, 2009, pp. 52-62.
