...
\begin{itemize}
    % subject data
    % macro instances, but no execution state -> no observer -> @see appendix
    % note: no kill behavior -> not needed, as no call macro / recursion
	\item \asminline{rule MacroBehavior} % minimal choose, no loop -> no execution state, no priorities
