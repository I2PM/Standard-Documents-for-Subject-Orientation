...
\begin{table}[htbp]
	...
	\begin{tabular}[t]{p{0.3\linewidth} p{0.6\linewidth}}
		...
		\\
		\hline
		Algorithms & \newline For computing period:\newline  
		\hspace*{40mm} {\large  $ \frac{\sum activity processing time} {\sum completed instances}\cdots \ldots$ } \tiny \newline
		\\
		\hline
		...
		\hspace*{4mm}(\textbf{SELECT} STARTTIME \newline
		\hspace*{4mm}\textbf{FROM} RT\_PROCINST \newline
		\hspace*{4mm}\textbf{WHERE} RT\_PROCDESC = \textit{process (purchase order)}\newline
		\hspace*{4mm}\textbf{AND} ID = \textit{instance (9)}\newline
		\textbf{FROM} REC\_RECTRANS\newline
		\textbf{WHERE} RT\_STDESC = \textit{\textit{state (fill\_in\_form)}}\newline
		\hspace*{4mm}\textbf{AND} RT\_PROCINST = \textit{instance (9)}
...
\begin{table}[htbp]
	\footnotesize
	\centering
	\begin{tabular}[t]{ p{0.1\linewidth} p{0.6\linewidth} p{0.3\linewidth}}
		\toprule
