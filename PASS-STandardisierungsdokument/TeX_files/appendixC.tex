...
rule SetCompletedAction(MI, currentStateNumber, res) =
  let ch = channelFor(self),
    ...
    SetCompleted(MI, s) // sets executionState to REPEAT
  }
\end{minted}
...
rule StartSend(MI, currentStateNumber) = {
  let pID = processIDFor(self) in
  let transitionNumber = first_outgoingNormalTransition(pID, currentStateNumber) in {
    receivers(channelFor(self), MI, currentStateNumber) := undef
    reservationsDone(channelFor(self), MI, currentStateNumber) := {}
    messageContent(channelFor(self), MI, currentStateNumber) := loadVar(MI, messageContentVar(pID, first_outgoingNormalTransition(pID, currentStateNumber)))

    let cIDVarname = messageNewCorrelationVar(pID, transitionNumber) in
...
rule SelectReceivers(MI, currentStateNumber) = {
  let pID = processIDFor(self) in
  let transitionNumber = first_outgoingNormalTransition(pID, currentStateNumber) in
  let countMin = messageSubjectCountMin(pID, transitionNumber),
    countMax = messageSubjectCountMax(pID, transitionNumber) in {
    if (messageSubjectVar(pID, transitionNumber) != undef and messageSubjectVar(pID, transitionNumber) != "") then {
      let rChs = loadChannelsFromVariable(MI, messageSubjectVar(pID, transitionNumber), messageSubjectId(pID, transitionNumber)) in {

        // countMin = 0 => ALL
        if (| rChs | = 0 or (countMin != 0 and | rChs | < countMin)) then {
          debuginfo SelectReceivers self + ": WARN: not enough receivers given in '" + messageSubjectVar(pID, transitionNumber) + "'"
      ...
      if (selectAgentsResult(channelFor(self), MI, currentStateNumber) != undef) then {
        ...
        let sIDLocal = messageSubjectId(pID, transitionNumber) in {
...
rule PerformTransitionSend(MI, currentStateNumber, transitionNumber) = {
  let pID = processIDFor(self) in {
    let storeReceiverVarname = messageStoreReceiverVar(pID, transitionNumber) in
...
rule DoReservation(MI, currentStateNumber, receiverChannel) = {
  if (properTerminated(receiverChannel) = true) then {
      ...
      let reservationMessage = [senderChannel, messageType(pID, transitionNumber), {}, msgCorrelationID, true] in {
        seq
          if (inputPool(receiverChannel, senderSubjectID, messageType(pID, transitionNumber), ipCorrelationID) = undef) then {
              add [senderSubjectID, messageType(pID, transitionNumber), ipCorrelationID] to inputPoolDefined(receiverChannel)
              inputPool(receiverChannel, senderSubjectID, messageType(pID, transitionNumber), ipCorrelationID) := []
          }
        next
          if (inputPoolIsClosed(receiverChannel, senderSubjectID, messageType(pID, transitionNumber), ipCorrelationID) != true) then {
              if (inputPoolGetFreeSpace(receiverChannel, senderSubjectID, messageType(pID, transitionNumber)) > 0) then {
                enqueue reservationMessage into inputPool(receiverChannel, senderSubjectID, messageType(pID, transitionNumber), ipCorrelationID)
...
rule ReceiveMessage(MI, currentStateNumber, transitionNumber) = {
  ...
  let s           = messageSubjectId         (pID, transitionNumber),
      sChsVarname = messageSubjectVar        (pID, transitionNumber),
      mt          = messageType              (pID, transitionNumber),
      cIDVarname  = messageWithCorrelationVar(pID, transitionNumber) in
